...
if ($tecnicoId > 0) {
  ...
  if ($stmtEmpresas) {
    $stmtEmpresas->bind_param('ii', $tecnicoId, $tecnicoId);
  ...
  if ($stmtPlanes) {
    $stmtPlanes->bind_param('ii', $tecnicoId, $tecnicoId);
    $stmtPlanes->execute();
  ...
  if ($stmtMantenimiento) {
    $stmtMantenimiento->bind_param('ii', $tecnicoId, $tecnicoId);
...
if (in_array($view, ['privada', 'reuniones'], true) && $tecnicoId > 0) {
  ...
  if ($stmtClientesEmpresa) {
    $stmtClientesEmpresa->bind_param('ii', $tecnicoId, $tecnicoId);
  ...
  if ($stmtTodasReuniones) {
    $stmtTodasReuniones->bind_param('iiii', $tecnicoId, $tecnicoId, $tecnicoId, $tecnicoId);
