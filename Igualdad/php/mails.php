...
function correo_crear_mailer(): PHPMailer
{
    ...
    // Habilitar debug SMTP si MAIL_DEBUG está configurado en el entorno (.env)
    $debugEnv = env('MAIL_DEBUG', '0');
    if (is_numeric($debugEnv) && (int)$debugEnv > 0) {
        $mail->SMTPDebug = (int)$debugEnv;
        // Enviar la salida de debug al error_log para poderla revisar en los logs de PHP
        $mail->Debugoutput = static function ($str, $level) {
            error_log('[PHPMailer-debug] ' . trim((string)$str));
        };
    }
...
function correo_enviar_html(
    ...
    // Permitir al callback de configuración modificar el mail (por ejemplo, añadir embedded images)
    if ($configurar !== null) {
        try {
            $configurar($mail);
        } catch (\Throwable $e) {
            error_log('[correo_enviar_html] Error en configurar callable: ' . $e->getMessage());
        }
    }
...
function correo_enviar_contacto_tecnico_empresa(
    ...
    correo_enviar_html(
        ...
        $body,
        $altBody,
        static function (PHPMailer $mail) use ($tecnicoEmail, $tecnicoNombre, $tecnicoFirma): void {
            ...
            if ($tecnicoFirma !== '') {
                ...
                if (
                    ...
                    try {
                        ...
                        error_log('Error embebiendo firma en email: ' . $e->getMessage());
                    }
                }
            }
        }
                    ...
                    if ($tecnicoFirma !== '') {
                        ...
                        if (
                            ...
                            try {
                                ...
                                error_log('Error embebiendo firma en email (copia tecnico): ' . $e->getMessage());
                            }
                        }
                    }
                }
