...
if ($rol === 'CLIENTE') {
  $stmt = $db->prepare("SELECT 1 FROM archivos a JOIN cliente_medida cm ON cm.id_cliente_medida = a.id_cliente_medida JOIN areas_contratadas ac ON ac.id_areas_contratadas = cm.id_areas_contratadas JOIN usuario_empresa ue ON ue.id_empresa = ac.id_empresa WHERE a.id_archivo = ? AND ue.id_usuario = ? LIMIT 1");
