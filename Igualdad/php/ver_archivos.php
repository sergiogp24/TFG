...
if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST' && ($accion = (string)($_POST['accion'] ?? '')) === 'eliminar_archivo') {
  if (!$esAdmin && !$esTecnico) {
    ...
    try {
      if ($source === 'archivos' && $fileId > 0) {
      ...
      } elseif ($source === 'uploads' && $fileName !== '') {
        // Eliminar archivo del disco
        $basePath = realpath(__DIR__ . '/..');
        $fullPath = $basePath . DIRECTORY_SEPARATOR . 'uploads' . DIRECTORY_SEPARATOR . $fileName;
        ...
        if (!$realFullPath || !$realBasePath || strpos($realFullPath, $realBasePath) !== 0) {
          ...
          if ($msgError === '' && file_exists($fullPath) && is_file($fullPath)) {
            if (@unlink($fullPath)) {
              $rutaRelativaDb = 'uploads/' . $fileName;
...
while ($row = $res->fetch_assoc()) {
  $idEmpresaRowResuelta = (int)($row['empresa_id_resuelta'] ?? 0);
  ...
  if (!$esAdmin) {
    if ($idEmpresaFiltro > 0) {
      // Si ya se validó el id_empresa en la URL, filtrar por ese contexto con fallback por nombre.
      if ($idEmpresaRow !== $idEmpresaFiltro && !archivo_subido_misma_empresa($empresaNombreRow, $empresaFiltroNombre)) {
        continue;
      }
    } else {
      if ($idEmpresaRow <= 0 || !isset($idsEmpresaPermitidas[$idEmpresaRow])) {
        continue;
      }
    }
  }

  if ($idEmpresaFiltro > 0 && $idEmpresaRow !== $idEmpresaFiltro && !archivo_subido_misma_empresa($empresaNombreRow, $empresaFiltroNombre)) {
    continue;
  }
  ...
  if ($rutaRelativa !== '') {
    ...
    $rutaNormDb = strtolower(str_replace('\\', '/', $rutaRelativa));
    if ($archivoDisponible) {
      $rutasYaIncluidas[$rutaNormDb] = true;
    }

    // Mantener filas de BD incluso cuando el tipo venga vacío, para no perder
  ...
  $archivosListado[] = [
    ...
    'descarga' => $archivoDisponible
      ? app_path('/php/download_archivo_subido.php?kind=archivos&id=') . $idArchivo
      : '#',
  ];
}
$stmt->close();
  ...
  foreach ($files as $fullPath) {
    ...
    if ($kind === 'uploads') {
      ...
      if (strpos($stem, '_TOMA_DE_DATOS_') !== false) {
        ...
        continue;
      } elseif (str_ends_with($stem, '_PLAN_IGUALDAD')) {
        // Word generado
        $stem = substr($stem, 0, -strlen('_PLAN_IGUALDAD'));
        $stem = archivo_subido_quitar_sufijo_anio($stem);
    ...
    $archivosListado[] = [
      ...
      'file_name' => $basename,
      'sort_ts' => $fileMtime,
      'subido_en' => $fileMtime > 0 ? date('Y-m-d H:i:s', $fileMtime) : '',
      'categoria' => $categoria,
