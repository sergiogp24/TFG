 ...
 */
    ...
    $tipoContratoFiltro = (string)($_GET['tipo_contrato'] ?? '');
    $volverLink = '';
    $volverLinkText = '';
        ...
        <div class="container-fluid<?= $soloMedidasEmbedMode ? ' py-2 px-2' : ' py-4' ?>">
            <div class="row g-3">
                ...
                <?php if (!$soloMedidasEmbedMode): ?>
                    <!-- Fragmento: barra lateral compartida (navegación de usuario) -->
                    <?php include __DIR__ . '/../php/fragments/sidebar.php'; ?>
                <?php endif; ?>

                <!-- MAIN CONTENT -->
                <main class="<?= $soloMedidasEmbedMode ? 'col-12' : 'col-12 col-md-9 col-xl-10' ?>">
                    <div class="card panel shadow-sm border-0">
                        <div class="card-body p-4">
                            <?php if (!$soloMedidasEmbedMode): ?>
                                <!-- HEADER DENTRO DE LA TARJETA -->
                                <div class="mb-4">
                                    <h2 class="fw-bold mb-1">Gestión de Empresas</h2>
                                    <p class="text-muted small mb-0">Administra los planes, contratos y documentos de tus empresas</p>
                            ...
                            <?php if ($view === 'ver_empresas'): ?>
                                ...
                                <div class="d-flex flex-wrap justify-content-between align-items-center gap-3 mt-3">
                                    ...
                                    <!-- Navegación de paginación: enlaces a páginas anteriores/siguientes del listado de empresas -->
                                ...
                                <?php else: ?>
                                    ...
                                    <div class="card border-0 shadow-sm mb-3 ge-company-card">
                                        <div class="card-body">
                                            ...
                                            <!-- Menú rápido de acciones relacionadas con la empresa: planes, contratos, archivos y subida de documentación -->
                                            <div class="ge-action-grid">
                                                ...
                                                <?php else: ?>
                                                    ...
                                                    <?php if (!$isAdmin): ?>
                                                            <!-- Enlace a planes: muestra planes asociados a la empresa (si procede) -->
                                                ...
                                                <a class="btn btn-dark btn-sm ge-company-action-btn" href="<?= h(app_path('/html/subir_documento_tipo.html.php?id_empresa=' . $detalleEmpresaId)) ?>">📤 Subir documentación</a>
                                                <?php if (!$isAdmin): ?>
                                    ...
                                    <div class="ge-tech-box mb-3">
                                        <span class="ge-tech-label">Personal Técnico asignados</span>
                                        <?php if (empty($detalleTecnicos)): ?>
                                            <strong class="ge-tech-value">Sin personal técnico asignado</strong>
                                        <?php else: ?>
                                    ...
                                    <?php else: ?>
                                        <?php
                                        // Preparar lista de áreas sin duplicados
                                        // y mapear nombres a rutas de mantenimiento específicas más abajo.
                                        $areasFlat = [];
                                        ...
                                        <?php else: ?>
                                            <div class="ge-area-grid">
                                                <?php foreach ($areasFlat as $nombreArea): ?>
                                                    <?php
                                                    // Mapeo heurístico: nombres de área => rutas de mantenimiento
                            ...
                            <?php elseif ($view === 'add_empresas'): ?>
                                ...
                                <!-- Formulario para crear una nueva empresa. Envía a controller/empresa_controller.php?action=add_empresas -->
                                ...
                                <form method="post" action="../controller/empresa_controller.php" class="vstack gap-2" enctype="multipart/form-data">
                                    ...
                                    <div>
                                        ...
                                        <!-- Widget CNAE: permite añadir múltiples códigos CNAE que se serializan en el textarea oculto -->
                            ...
                            <?php elseif ($view === 'edit_empresas'): ?>
                                ...
                                <div class="d-flex justify-content-between align-items-center mb-4">
                                    ...
                                    <!-- Formulario de edición: al enviar actualiza datos y archivos de la empresa -->
                    ...
                    <?php elseif ($view === 'ver_planes'): ?>
                        ...
                        <!-- Sección: Planes asignados a empresas (lista y acciones) -->
                        <!-- VER PLANES -->
                        <div class="d-flex justify-content-between align-items-center mb-3">
                            ...
                            <div class="d-flex align-items-center gap-2">
                                ...
                                <?php if (($empresaContexto ?? null) !== null): ?>
                                    <a class="btn btn-secondary btn-sm" href="../model/empresa.php?view=ver_empresa&id_empresa=<?= (int)($empresaContexto['id_empresa'] ?? 0) ?>">Volver a la empresa</a>
                                <?php endif; ?>
                            </div>
                        </div>
                    ...
                    <?php elseif ($view === 'ver_medidas'): ?>
                        ...
                        <!-- Sección: Áreas y medidas del plan (vista de solo lectura) -->
                    ...
                    <?php elseif ($view === 'edit_plan'): ?>
                        ...
                        <!-- Sección: Editar plan - formulario para modificar fechas y medidas -->
                        ...
                        <?php else: ?>
                            ...
                            <form method="post" action="../controller/empresa_controller.php" class="vstack gap-3">
                                ...
                                <div class="border rounded p-3 bg-light">
                                    ...
                                    <?php $editAreasPlan = $editAreasPlan ?? []; ?>
                                    <?php $editAreasSeleccionadas = $editAreasSeleccionadas ?? []; ?>
                    ...
                    <?php elseif ($view === 'ver_contratos'): ?>
                        ...
                        <!-- Sección: Contratos y mantenimientos asociados a empresas -->
                        <div class="d-flex justify-content-between align-items-center mb-3">
                            <h6 class="mb-0"><?= (($tipoContratoFiltro ?? '') === 'MANTENIMIENTO') ? 'Mantenimientos de empresas' : 'Servicios aceptados' ?></h6>
                            <div class="d-flex align-items-center gap-2">
                                <?php if (($empresaContexto ?? null) !== null): ?>
                                    <a class="btn btn-secondary btn-sm" href="../model/empresa.php?view=ver_empresa&id_empresa=<?= (int)($empresaContexto['id_empresa'] ?? 0) ?><?= $fromParam ?>">Volver a la empresa</a>
                                <?php endif; ?>
                                <?php if (!$isTecnico): ?>
                                    <?php $__tipoContrato = isset($tipoContratoFiltro) ? $tipoContratoFiltro : ''; ?>
                                    <a class="btn btn-primary btn-sm" href="../model/empresa.php?view=add_contratos<?= ($idEmpresaContexto > 0) ? '&id_empresa=' . (int)$idEmpresaContexto : '' ?><?= ($__tipoContrato !== '') ? '&tipo_contrato=' . urlencode((string)$__tipoContrato) : '' ?><?= $fromParam ?>"><?= ($__tipoContrato === 'MANTENIMIENTO') ? 'Añadir Mantenimiento' : (($__tipoContrato === 'PLAN IGUALDAD') ? 'Añadir Plan igualdad' : 'Añadir Servicios') ?></a>
                                <?php endif; ?>
                            </div>
                        </div>
                            ...
                            <div class="contratos-table-wrap">
                                <table class="table table-bordered align-middle text-center contratos-table">
                                    ...
                                    <tbody>
                                        ...
                                        $tipoContratoFiltro = $tipoContratoFiltro ?? '';
                                        $idEmpresaContexto = $idEmpresaContexto ?? 0;
                                        $fromParam = $fromParam ?? '';
                                        $fromPanel = $fromPanel ?? '';
                                        $i = 1 + (((int)($currentPageContratos ?? 1) - 1) * 10);
                                        ?>
                        ...
                        <?php else: ?>
                            <form method="post" action="../controller/empresa_controller.php" class="vstack gap-2">
                                ...
                                <?php if (!$soloMedidasEmbedMode): ?>
                                    ...
                                    <div>
                                        <label class="form-label">Personal Técnico asignado al servicio</label>
                                        <?php if ($isTecnico): ?>
                                        ...
                                        <?php else: ?>
                                            <select class="form-select" name="id_usuario" required>
                                                <option value="">-- seleccionar Personal Técnico --</option>
