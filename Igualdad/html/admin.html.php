...
<body class="bg-light">
                ...
                <div class="card panel shadow-sm border-0 <?= in_array($view, ['menu', 'ver_usuarios'], true) ? 'panel-wide' : '' ?> <?= ($view === 'seguimiento_tecnicos') ? 'panel-followup' : '' ?> <?= ($view === 'reuniones') ? 'panel-reuniones' : '' ?>">
                    <div class="card-body p-4">
                                        ...
                                        <article class="operational-item">
                                            <div class="operational-top">
                                                <div>
                                                    ...
                                                    <div class="operational-meta"><?= h((string)($item['plan'] ?? '')) ?> · Personal Tecnico: <?= h((string)($item['tecnico'] ?? 'Sin tecnico asignado')) ?></div>
                                                </div>
                        ...
                        <?php elseif ($view === 'seguimiento_tecnicos'): ?>
                            ...
                            <div class="seguimiento-wrap">
                                <div class="d-flex justify-content-between align-items-start flex-wrap gap-2">
                                    <div>
                                        <h4 class="mb-1">Seguimiento por Personal Técnico</h4>
                                        <div class="text-muted">Selecciona un Personal Técnico para ver sus empresas asignadas y el avance.</div>
                                    </div>
                                </div>

                                <?php if (empty($seguimientoTecnicos)): ?>
                                    <div class="alert alert-light border mt-3 mb-0">No hay Personal Técnico asignados para mostrar.</div>
                                <?php else: ?>
                                    ...
                                    <div class="seguimiento-detalle">
                                        ...
                                        <?php if (empty($seguimientoTecnicoEmpresas)): ?>
                                            <div class="alert alert-light border mb-0">Este Personal Técnico no tiene empresas asignadas.</div>
                                        <?php else: ?>
                        ...
                        <?php elseif ($view === 'add'): ?>
                            ...
                            <form method="post" action="../controller/admin_controller.php" class="vstack gap-2" enctype="multipart/form-data">
                                <?= csrf_input() ?>
                        ...
                        <?php elseif ($view === 'edit'): ?>
                            ...
                            <?php else: ?>
                                <form method="post" action="../controller/admin_controller.php" class="edit-user-form" enctype="multipart/form-data">
                                    <?= csrf_input() ?>
                                    ...
                                    <div class="row g-4">
                                        ...
                                        <div class="col-12 col-md-4">
                                            ...
                                            <select class="form-select edit-input" id="editUserRol" name="rol_id" required>
                                                <?php foreach (($roles ?? []) as $r): ?>
                                                    <?php
                                                    $rolNombreNormalizado = strtoupper(str_replace(['Á', 'É', 'Í', 'Ó', 'Ú'], ['A', 'E', 'I', 'O', 'U'], trim((string)($r['nombre'] ?? ''))));
                                                    ?>
                                                    <option value="<?= (int)$r['id'] ?>" data-role-normalized="<?= h($rolNombreNormalizado) ?>" <?= ((int)$r['id'] === (int)($selectedUser['rol_id'] ?? 0)) ? 'selected' : '' ?>>
                                                        <?= h($r['nombre']) ?>
                                                    </option>
                                                <?php endforeach; ?>
                                            </select>
                                        ...
                                        <div class="col-12" id="editUserFirmaWrap">
                                            <label class="form-label text-center w-100">Firma correo (imagen)</label>
                                            <input class="form-control edit-input" name="firmacorreo" type="file" accept=".png,.jpg,.jpeg,.webp,.gif,image/png,image/jpeg,image/webp,image/gif">
                                            <?php $firmaActual = trim((string)($selectedUser['firmacorreo'] ?? '')); ?>
                                            <?php if ($firmaActual !== ''): ?>
                                                <div class="form-text mt-2">
                                                    Firma actual:
                                                    <a href="<?= h(app_path('/' . ltrim($firmaActual, '/'))) ?>" target="_blank" rel="noopener">ver imagen</a>
                                                </div>
                                            <?php else: ?>
                                                <div class="form-text mt-2">No hay firma configurada.</div>
                                            <?php endif; ?>
                                        </div>
    ...
    <script>
        ...
        (function() {
            ...
            const addFirmaWrap = document.getElementById('addUserFirmaWrap');
            const addFirmaInput = addFirmaWrap ? addFirmaWrap.querySelector('input[name="firmacorreo"]') : null;

            const editRoleSelect = document.getElementById('editUserRol');
            const editFirmaWrap = document.getElementById('editUserFirmaWrap');
            const editFirmaInput = editFirmaWrap ? editFirmaWrap.querySelector('input[name="firmacorreo"]') : null;

            function isTecnicoRole(option) {
                const role = String(option?.dataset?.roleNormalized || '').toUpperCase();
            ...
            function updateCompanyRequirementByRole() {
                ...
                const isTecnico = selectedRole.includes('TECNICO');
            ...
            function updateEditFirmaVisibility() {
                if (!editRoleSelect || !editFirmaWrap) {
                    return;
                }

                const selectedOption = editRoleSelect.options[editRoleSelect.selectedIndex] || null;
                const isTecnico = isTecnicoRole(selectedOption);
                editFirmaWrap.style.display = isTecnico ? '' : 'none';
                if (!isTecnico && editFirmaInput) {
                    editFirmaInput.value = '';
                }
            }
